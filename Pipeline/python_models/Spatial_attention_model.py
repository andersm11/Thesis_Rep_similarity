...
class SpatialAttention(nn.Module):
    def __init__(self, in_channels: int):
        super().__init__()
        # Query, Key, and Value are now computed over the spatial dimension (electrodes)
        ...
        self.out_linear = nn.Linear(in_channels, in_channels, bias=False)  
        self.scale = nn.Parameter(torch.tensor(1.0 / (in_channels ** 0.5)))

    def forward(self, x):
        B, temp_out_channels, spat_channels, time_steps = x.shape
        # Reshape to: [B * temp_out_channels, spat_channels, time_steps]
        x_reshaped = x.view(B * temp_out_channels, spat_channels, time_steps)

        # Compute Queries, Keys, and Values over the spatial dimension (electrodes)
        print(x_reshaped.shape)
        x_reshaped = x_reshaped.permute(0,2,1)
        print(x_reshaped.shape)
        query = self.query(x_reshaped)  # Query: [B * temp_out_channels, spat_channels, time_steps]
        key = self.key(x_reshaped)      # Key: [B * temp_out_channels, spat_channels, time_steps]
        value = self.value(x_reshaped)  # Value: [B * temp_out_channels, spat_channels, time_steps]

        # Compute attention scores (spatial attention): [B * temp_out_channels, spat_channels, time_steps]
        attn_scores = torch.einsum("bct,bcs->bts", query, key) * self.scale  # [B * temp_out_channels, time_steps, time_steps]
        attn_weights = torch.softmax(attn_scores, dim=-1)  # [B * temp_out_channels, time_steps, time_steps]

        # Apply attention to the value tensor
        out = torch.einsum("bts,bcs->bct", attn_weights, value)  # [B * temp_out_channels, spat_channels, time_steps]
...
class ShallowAttentionNet(nn.Module):
    def __init__(self, n_chans, n_outputs, n_times, dropout=0.7, num_kernels=10, kernel_size=25, pool_size=100):
