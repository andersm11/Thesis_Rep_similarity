    ...
    def __init__(
        ...
        if self.split_first_layer:
            ...
            self.conv_time = nn.Conv2d(
                1, n_filters_time, (filter_time_length, 1), bias=True, stride=1
            )
            self.conv_spat = nn.Conv2d(
                n_filters_time, n_filters_spat, (1, 22), bias=not batch_norm, stride=1
            )
        ...
        init.xavier_uniform_(self.conv_time.weight, gain=0.5)
        # maybe no bias in case of no split layer and batch norm
        if self.split_first_layer or (not self.batch_norm):
            init.constant_(self.conv_time.bias, 0)
        if self.split_first_layer:
            init.xavier_uniform_(self.conv_spat.weight, gain=0.5)
